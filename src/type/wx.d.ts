...
// declare let console: Console;
 ...
 declare namespace wx {
     ...
     interface systemInfo {
       safeArea: {top: number; left: number;}; // todo 不全
       /** 手机品牌*/
       brand: string;
       /** 手机型号*/
       model: string;
     ...
     /**
      * banner 广告组件。banner 广告组件是一个原生组件，层级比上屏 Canvas 高，会覆盖在上屏 Canvas 上。
      * banner 广告组件默认是隐藏的，需要调用 BannerAd.show() 将其显示。banner 广告会根据开发者设置的宽度进行等比缩放，缩放后的尺寸将通过 BannerAd.onResize() 事件中提供。
      */
     interface BannerAd {
       /**
     ...
     interface SocketTask {
       ...
       /**
          * 监听WebSocket 连接关闭事件
          */
       onClose(callback: (err: any) => void): void;
       /**
          * 监听WebSocket 错误事件
          */
     ...
     interface UDPSocket {
        ...
        * 并且 message 事件回调函数也不会再也执行。在 UDPSocket 实例被创建后将被 Native 强引用，
        * 保证其不被 GC。在 UDPSocket.close 后将解除对其的强引用，让 UDPSocket 实例遵从 GC。
        * */
       close(): void;
       /** 取消监听关闭事件*/
     ...
     /**
      * 通过 wx.login 接口获得的用户登录态拥有一定的时效性。用户越久未使用小程序，用户登录态越有可能失效。反之如果用户一直在使用小程序，
      * 则用户登录态一直保持有效。具体时效逻辑由微信维护，对开发者透明。开发者只需要调用 wx.checkSession 接口检测当前用户登录态是否有效。登录态过期后开发者可以再调用 wx.login 获取新的用户登录态。
      */
     ...
     // eslint-disable-next-line @typescript-eslint/unified-signatures,@typescript-eslint/adjacent-overload-signatures
     function getUserInfo(object: GetUserInfoOpt): void;
     ...
     interface OpenDataContext {
       ...
       /**
          * 向开放数据域发送消息
          * @param message {} 要发送的消息，message 中及嵌套对象中 key 的 value 只能是 primitive value。
          * 即 number、string、boolean、null、undefined。
          */
     ...
     /**
      * 对于游戏来说，每帧 16ms 是极其宝贵的，如果有一些可以异步处理的任务，可以放置于 Worker 中运行，待运行结束后，
      * 再把结果返回到主线程。Worker 运行于一个单独的全局上下文与线程中，不能直接调用主线程的方法，Worker 也不具备渲染的能力。
